...
export function shouldFetchGithubReleases({
  force = false,
  hasResolvedSnapshot = false,
} = {}) {
  return Boolean(force || !hasResolvedSnapshot);
}

export function shouldLoadBuildReleaseSnapshot({
  force = false,
  hasFreshCache = false,
} = {}) {
  return Boolean(!force && !hasFreshCache);
}

export function getReleaseUpdatedLabel(source) {
