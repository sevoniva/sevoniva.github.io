---
title: 监控与可观测性
description: Crest v1.5.4 Prometheus 指标、Grafana 看板、启用方式、指标口径和安全边界。
---

Crest v1.5.4 支持按需接入 Prometheus 和 Grafana。默认安装不会启动监控组件，后端也不会开放 Prometheus 指标端点；需要监控时，由运维人员在部署配置中显式启用。
...
Crest v1.5.4 交付 6 个 Grafana 看板：
