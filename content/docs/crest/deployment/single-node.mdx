...
这种方式适合演示环境、内部小规模使用或单节点部署。Crest v1.5.4 单机安装会启动前端网关 `crest`、后端服务 `crest-service` 和元数据库 `mysql-crest`；需要监控时，还可以同时启用 `crest-prometheus` 和 `crest-grafana`。正式使用前仍然要按生产标准准备域名、HTTPS、备份、密码和运维责任人。
...
CREST_BACKEND_IMAGE=ghcr.io/sevoniva/crest-service:v1.5.4
CREST_FRONTEND_IMAGE=ghcr.io/sevoniva/crest-web:v1.5.4
