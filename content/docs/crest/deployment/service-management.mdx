...
v1.5.7 单机部署中，`crest` 是前端 Nginx 网关，`crest-service` 是后端 Spring Boot 服务。页面无法访问时优先查看 `crest`；登录、接口、数据集、导出和权限异常时优先查看 `crest-service`。启用监控后，Prometheus 和 Grafana 日志分别查看：
