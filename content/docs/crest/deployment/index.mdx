...
本章面向负责安装、交付和运维 Crest 的人员，覆盖部署前准备、安装过程、服务管理、外部数据库、Kubernetes 和上线检查。Crest v1.5.4 采用前后端分离运行形态：前端网关、后端服务和元数据库分别运行，监控组件可按需启用。正式上线时，应同时确认访问地址、数据库、密钥、备份、日志、监控和权限治理，避免系统虽然可访问但不具备长期运行条件。
...
| 单机离线安装 | 服务器不能访问外网 | 中 | 先制作包含前端、后端、MySQL 和可选监控镜像的离线包，再上传到目标服务器执行安装 |
...
<Cards>
  ...
  <Card title="监控与可观测性" href="/docs/crest/maintenance/observability">
    说明 v1.5.4 的 Prometheus 指标、Grafana 看板和告警口径。
  </Card>
</Cards>
