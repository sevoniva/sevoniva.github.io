...
本章面向负责安装、交付和运维 Crest 的人员，覆盖部署前准备、安装过程、服务管理、外部数据库、Kubernetes 和上线检查。正式上线时，应同时确认访问地址、数据库、密钥、备份、日志和权限治理，避免系统虽然可访问但不具备长期运行条件。

## 选择部署方式

| 部署方式 | 推荐场景 | 维护成本 | 说明 |
| --- | --- | --- | --- |
| 单机在线安装 | 有网络访问镜像仓库，需要完成单节点部署 | 低 | 使用 `installer/install.sh` 自动安装 Docker、配置目录和系统服务 |
| 单机离线安装 | 服务器不能访问外网 | 中 | 先制作离线包，再上传到目标服务器执行安装 |
| Kubernetes 内置 MySQL | 集群内验证或测试 | 中 | 同时部署 Crest 和 MySQL StatefulSet |
| Kubernetes 外部 MySQL | 正式集群环境 | 高 | 推荐生产使用，数据库由平台统一维护 |
| 自定义 Docker Compose | 已有统一运维模板 | 中 | 参考安装包中的 Compose 模板自行纳管 |

<Callout type="warning" title="生产环境建议">
  正式环境优先选择固定域名、HTTPS、外部 MySQL、定期备份和统一日志采集。不要把 Crest、数据库和备份文件都当成临时测试资源处理。
</Callout>
...
flowchart TB
  ...
  C --> D["配置初始密码、数据库密码、AES Key 和允许来源"]
  D --> E["启动 Crest 服务"]
  E --> F["登录系统并修改管理员密码"]
...
| 密码与密钥 | 初始化密码、数据库密码、AES Key 和 AES IV 必须可追溯、可备份 |
| 备份 | 明确数据库和运行目录备份位置 |
| 日志 | 明确日志保留周期和排查方式 |
