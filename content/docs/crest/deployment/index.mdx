...
本章面向负责安装、交付和运维 Crest 的人员，覆盖部署前准备、安装过程、服务管理、外部数据库、Kubernetes 和上线检查。Crest v1.5.6 采用前后端分离运行形态：前端网关、后端服务和元数据库分别运行，监控组件可按需启用。正式上线时，应同时确认访问地址、数据库、密钥、备份、日志、监控和权限治理，避免系统虽然可访问但不具备长期运行条件。
