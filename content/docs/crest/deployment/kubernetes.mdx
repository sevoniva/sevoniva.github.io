...
Kubernetes 部署更适合已有集群和运维体系的团队。它的重点不是把 Pod 拉起来，而是把命名空间、Secret、PVC、Service、Ingress、数据库、日志、备份和监控都纳入统一管理。首次验证可以用内置 MySQL，生产环境建议优先评估外部 MySQL。

集群部署完成后，仍然要回到页面验证，而不是只看 Pod 状态。下面的工作台截图可作为第一轮页面验收参考：能登录、能加载菜单、能显示资源统计，才说明前端网关、后端服务和元数据库基本连通。
...
3. `kubectl logs` 先看后端 `crest-service`，再看前端网关 `crest`。
