...
这页只解决一件事：开发者第一次拿到 Crest `v1.5.4` 源码后，怎么在本机把项目跑起来，并且知道哪些配置该改、哪些配置不该动。

开发环境不要求本机有 Docker。只要本机或内网有一套可连接的 MySQL 8.x，就可以启动后端和前端。v1.5.4 已经前后端分离，如果你只改后端，可以复用发布的前端镜像或已有前端环境；如果只改前端，可以复用发布的后端镜像或内网后端服务。
...
`v1.5.4` 是本页对应的版本。不要拿 `main`、其他功能分支或旧 dump 混在一起排查，否则数据库结构和代码很容易对不上。
...
本机没有 Docker 时，不影响前端或后端源码开发。镜像构建可以交给 CI、构建机或有 Docker 的同事环境。
...
如果你只是打后端 JAR，优先加上 `-Dcrest.copy.frontend.skip=true`。只有打内嵌静态资源 JAR 或构建前端镜像时，才需要先生成前端 `dist`。
